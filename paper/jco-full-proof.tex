...
\end{lemma}
\begin{proof}
First we realize that by decreasing $x_{ij}$, the objective value can not increase. This variable can be reduced as long as it preserves feasibility of the solution. The only constraints that could be violated by reducing $x_{ij}$ are (\ref{con:pf1:xfrel}) and (\ref{con:pf1:flowX}), and so asserting 
$$x_{ij}=\max\bigg\{ f^*_{ij},\sum_{k\in V_i}x_{ki}-\sum_{k\in V_i\setminus\{j\}}x_{ik}\bigg\}$$
either does not change the value $x_{ij}$, or yields another optimum. \qed
\end{proof}
...
\begin{figure}[h!]
        \centering
        \includegraphics[height=2.3in]{slack-cut}
...
contradicting either (\ref{eq:sumToD}), if $i\in D_0$, or (\ref{con:pf1:B}), if $i\in V\setminus D$. Now let us assume %that $f^*_{ij}<x_{ij}$ and
 (\ref{con:pf1:flowX}) is satisfied with equality. Adding $x_{ji}$ to both sides of the inequality (\ref{con:mfinpf2:arrowFromNonDestA}) and utilizing (\ref{con:pf1:B}) gives the desired relation
\[
x_{ij}+x_{ji}\leq\sum_{k\in V_i\setminus\{j\}}x_{ki} +x_{ji}=\sum_{k\in V_i}x_{ki}\leq 1~~~~~~~i\in V\setminus D.
\]
