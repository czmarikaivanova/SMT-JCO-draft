...
Consider a multi-commodity network flow problem where one unit of commodity $(s,t)$ must be sent from $s\in D$ and $t\in D$. For this purpose, let $S=\{(s,t)\in D\times D, s\neq t\}$ be the set of ordered pairs of distinct destinations. In order to model the connectivity requirements, we introduce a variable $f^{st}_{ij}$ as follows:
\newline\newline
  $f_{ij}^{st}=
	\begin{cases}
...
SMTO can be extended and strengthened by flow conservation constraints for each $(s,t)$-pair, which gives us
\newline
\newline    
\begin{subequations}
\begin{flalign}
...
\newline\newline  
  $f^{t}_{ij}=
	\begin{cases}
    1 & \text{if arc $(i,j) \in A$ carries flow from $v_0$ to $t\in D_1$},\\
    0 & \text{otherwise}.
  \end{cases}$  
\newline\newline  
  $x_{ij}=
...
\begin{align}
\label{eq:tr:xijj1}x^s_{ij}&= x_{ij}-f^s_{ij} + f^{s}_{ji} & (i,j)\in A, s\in D_1
\end{align}
...
%&=  f^t_{ij}+f^s_{ji} - \check{f}^{st}_{ij} - \check{f}^{st}_{ji}& (i,j)\in A, \{s,t\}\in S_1\\
%\notag\label{eq:tr:xijj}x^s_{ij}&=x_{ij}(1-f^{s}_{ij})(1-f^{s}_{ji})+x_{ji}f^{s}_{ji}=
%\\&= x_{ij}-f^s_{ij} + f^{s}_{ji} & (i,j)\in A, s\in D_1\\
...
%\label{eq:tr:fijt2}f^t_{ij}=&x^t_{ji}x^0_{ij}=f^{0t}_{ij} & (i,j)\in A, t\in D_1\\
%\label{eq:tr:fstij2}\check{f}^{st}_{ij}=& x^s_{ji}x^t_{ji}x^0_{ij} & (i,j)\in A, \{s,t\}\in S_1
%\end{align}
%\end{subequations}
...
Having this transformation in hand, it is easy to construct a SMT model based on the minimum Steiner tree model PF1:
  ...
  \min &  \sum\limits_{(i,j) \in A} \sum\limits_{s \in D} p_{ij} y^s_{ij}    \\  \notag  
		   \text{s.t.}&                  && \\	\ 
\label{con:pf1:xfrel}  f^{t}_{ij}   &\leq x_{ij}    ~~ \qquad \qquad t\in D_1, (i,j)\in A \\
 \label{con:pf1:flow}  \sum\limits_{\substack{ j \in V_i }}f^{t}_{ji}-\sum\limits_{\substack{j\in V_i}}f^{t}_{ij}    & = \begin{cases}
    1~~~~&  t\in D_1, t = i \\        0~~~~ \qquad             & t\in D_1, i\in V\setminus \{v_0, t\}\end{cases}     \\	
...
%    -1~~~&  \{s,t\}\in S_1, i = v_0 \\        0  ~~~~   \qquad    & \{s,t\}\in S_1, i\in V\setminus \{v_0\}\end{cases}     \\			
%\label{con:pf2:hookImpFs}  \check{f}^{st}_{ij}    & \leq f^s_{ij}~~   \qquad  \qquad \{s,t\}\in S_1, (i,j)\in A \\	
%\label{con:pf2:hookImpFt}  \check{f}^{st}_{ij}    & \leq f^t_{ij}~~   \qquad    \qquad \{s,t\}\in S_1, (i,j)\in A \\			 		
% \label{con:pf2:stronger}  f^{s}_{ij}+f^{t}_{ij}-\check{f}^{st}_{ij}    &\leq x_{ij}    ~~ \qquad \qquad  \{s,t\}\in S_1, (i,j)\in A \\			 
...
\label{con:pf1:noflowFromT} f_{ti}^t&=0 ~~~~ \qquad  \qquad t\in D_1, i\in V_t   \\
\label{con:pf1:fitt=xit} f_{it}^t&=x_{it} ~~ \qquad  \qquad t\in D_1, i\in V_t \\
\label{con:pf1:xi0=0} x_{i0}&=0 ~~~~ \qquad  \qquad i\in V_0 \\
\label{con:pf1:fi0s=0} f_{i0}^t&=0 ~~~~ \qquad  \qquad i\in V_0, t\in D_1 \\
\label{con:pf1:fij0=0} f_{ij}^0&=0 ~~~~ \qquad  \qquad (i,j)\in A \\	   			  \label{con:pf1:dim}	\mathbf{x} \in \{0,1\}^{A},\mathbf{f}&\in\{0,1\}^{A \times D} \\ 
\label{con:pf1:dimy} \mathbf{y}&\in \{0,1\}^{A\times D}
    \end{flalign}~
    \end{subequations}
    
    Constraints (\ref{con:pf1:xfrel})-(\ref{con:pf1:flowX}) together with (\ref{con:pf1:dim}) imply that $\mathbf{x}$ induces and arborescence spanning $D$ with node $v_0$ as the root. The constraint (\ref{con:pf1:yvar}) has the same purpose as (\ref{con:dd:yvar}), and is expressed in SMTPF1 space using transformation (\ref{eq:tr:xijj}). Note that the $y_{ij}^s$-variables determining power levels are defined for all destinations $s\in D$, while in the PF1 model of Steiner minimum tree problem, the $f_{ij}^s$ variables are defined only for $s\in D_1$. This inconsistency is resolved by extending the domain of PF1 variables and fixing the newly introduced variables to zero, which is ensured by constraints (\ref{con:pf1:fi0s=0})-(\ref{con:pf1:fij0=0}).
\begin{figure}[!htb]
    \centering
...
Similarly to the extension of SMTO by the $s-t$-flow variables, the SMTPF1 model can also be extended by 4-index variables. Analogously to $S$, let $\check{S}=\{\{s,t\}\subseteq D: s\neq t\}$ be the set of unordered pairs of destinations, and let $\check{S}_1=\{\{s,t\}\in S: s\neq v_0\neq t\}$. Authors in \cite{Polzin} use variables

%\newline\newline  
  $\check{f}^{st}_{ij}=
	\begin{cases}
    1 & \text{if arc $(i,j) \in A$ carries flow from $v_0$ to both $s$ and $t$, $\{s,t\}\in \check{S}_1$},\\
    0 & \text{otherwise},
  \end{cases}$
  describing the common flow from $v_0$ to $s$ and $t$. This allows to create an extended  PF2 model by adding constraints relating to the $\check{f}^{st}_{ij}$-variables. An SMT model SMTPF2 based on PF2 has the following form:
...
\label{con:pf2:flowHook}  \sum\limits_{\substack{ j \in V_i }}\check{f}^{st}_{ji}-\sum\limits_{\substack{j\in V_i}}\check{f}^{st}_{ij}    & \geq \begin{cases}
    -1~~~&  \{s,t\}\in \check{S_1}, i = 0 \\        0  ~~~~   \qquad         & \{s,t\}\in \check{S_1}, i\in V\setminus \{v_0\}\end{cases}     \\			
\label{con:pf2:startInSource}  \check{f}^{st}_{ij}    & \leq f^s_{ij}~~   \qquad  \qquad \{s,t\}\in \check{S_1}, (i,j)\in A \\	
\label{con:pf2:stopInDest}  \check{f}^{st}_{ij}    & \leq f^t_{ij}~~   \qquad    \qquad \{s,t\}\in \check{S_1}, (i,j)\in A \\			 		
 \label{con:pf2:stronger}  f^{s}_{ij}+f^{t}_{ij}-\check{f}^{st}_{ij}    &\leq x_{ij}    ~~ \qquad \qquad  \{s,t\}\in \check{S_1}, (i,j)\in A \\			 	   			  
...
\label{eq:tr:fstij2}x_{ij}=&x^0_{ij} & (i,j)\in A\\
\label{eq:tr:fijt2}f^t_{ij}=&x^t_{ji}x^0_{ij}=f^{0t}_{ij} & (i,j)\in A, t\in D_1\\
\label{eq:tr:fstij2}\check{f}^{st}_{ij}=& x^s_{ji}x^t_{ji}x^0_{ij} & (i,j)\in A, \{s,t\}\in \check{S}_1
\end{align}
\end{subequations}
...
\label{con:mfinpf2:maxsize}   \sum\limits_{\{i,j\}\in E}(x_{ij}+x_{ji}) & \leq  N-1  &   && \\
     \label{con:mfinpf2:arrowFromDest}  \sum\limits_{j\in V_i}(x_{ji}-f^s_{ji}+f^s_{ij})          & = 1       			&& i\in D, s\in D_1, i \neq s && \\ 
		  \label{con:mfinpf2:arrowFromNonDestB}  \sum\limits_{j \in V_i}(x_{ji}-f^s_{ji}+f^s_{ij})  &\leq 1 && i\in V \setminus D, s\in D_1   &&\\		
		  \label{con:mfinpf2:arrowFromNonDestA}  x_{ik}-f^s_{ik}+f^s_{ki}  & \leq \sum\limits_{j \in V_i\setminus\{k\}}(x_{ji}-f^s_{ji}+f^s_{ij}) &&
		  i\in V \setminus D,k\in V_i, s\in D_1   &&\\		  
      \label{con:mfinpf2:extraCon}  \sum\limits_{j \in V_i}(x_{ji}-f^s_{ji}+f^s_{ij}) & \leq \sum\limits_{j \in V_i}(x_{kj}-f^s_{kj}+f^s_{jk}) &&  i\in V\setminus D, s\in D_1  &&\\			  \label{con:mfinpf2:startInSource}  x_{js} - f^s_{js}+f^s_{sj}    & = 0       			&&  s\in D_1, j\in V_s &&\\		 
		  \label{con:mfinpf2:yvar}  x_{ij}-f^s_{ij}+f^s_{ji} & \leq \sum\limits_{k\in V:p_{ik}\geq p_{ij}}y^s_{ik} && s\in D, (i,j)\in A 		&&\\  
		 		 \label{con:mfinpf2:flowNormal}  \sum\limits_{\substack{ j\in V_i }}f^t_{ij}-\sum\limits_{\substack{j\in V_i }}f^t_{ji}    & = 0     			&& i\in V, t\in D_1, i \neq t &&\\	
		 	 \label{con:mfinpf2:flowDest}  \sum\limits_{\substack{j\in V_t}}f^t_{tj}-\sum\limits_{\substack{j\in V_t}}f^t_{jt}    & = -1     			&&  t \in D_1 &&\\	
%		 			 	 \label{con:mfinpf2:flowSource}  \sum\limits_{\substack{ (s,j)\in A}}(f^t_{sj}+f^s_{js})-\sum\limits_{\substack{(j,s)\in A}}(f^t_{js}+f^s_{sj})    & = 1     			&&  \{s,t\}\subseteq D_1 && \\	
           \label{con:mfinpf2:fcap}   f^t_{ij} - \check{f}^{st}_{ij} - \check{f}^{st}_{ji} &\leq  x_{ij}-f^s_{ij}   && (i,j)\in A, \{s,t\}\in \check{S}_1 && &\\ 		 			 	 
		    \label{con:mfinpf2:zbound} x_{ij}+x_{ji}&\leq 1 &&\{i,j\}\in E \\
  		    \label{con:mfinpf2:xbound} 0\leq x_{ij}-f_{ij}^s+f_{ji}^s & \leq 1 && \{i,j\}\in E,  s\in D_1 \\
\label{con:mfinpf2:fbound} 0\leq f_{ij}^t+f_{ji}^s-\check{f}_{ij}^{st}-\check{f}_{ji}^{st} & \leq 1 && \{i,j\}\in E,\{s,t\}\in \check{S}_1 \\
...
The SMTMF model can be further extended by valid inequalities strengthening its LP bounds:
  \begin{subequations}
  \begin{flalign}
\label{con:vi:xImpY}  x^{s}_{ij} & \leq \sum\limits_{t\in D\setminus\{s\}}  f^{st}_{ij},  \quad\quad    (i,j)\in A,(s,t)\in S \\
 \label{con:vi:Y1}  \sum\limits_{j\in V_s}  y^{s}_{sj} & =1,  \quad\quad\quad\quad\quad\quad    s\in D \\
...
\end{equation}
because $S_1$ is a set of 2-element sets. By the implicit assumption of (\ref{eq:fhooksym}) in SMTPF2, it is possible to infer additional valid inequalities for SMTMF. We can also write
\begin{align*}
...
\begin{equation*}f^{ut}_{ij}+f^{us}_{ji}+f^{ts}_{ij}=f^{us}_{ij}+f^{ut}_{ji}+f^{st}_{ij} \quad\quad (u,t),(u,s),(s,t),(t,s)\in S_1, i,j\in V
\end{equation*} 
